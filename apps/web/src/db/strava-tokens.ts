...
/**
 * INSERT ... ON CONFLICT (user_id) DO UPDATE. athlete_strava_id has no
 * uniqueness constraint -- multiple users may link the same Strava
 * account, so this is a plain per-user upsert with no cross-user check.
 *
