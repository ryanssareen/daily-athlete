...
import { type IntensityTarget, type WorkoutMatchMethod } from "@da2/shared";
...
async function resolveMatch(
  ...
  return {
    ...
    load: readStructureLoad(row.structure, row.planned_load),
...
// Defensive structure reads (the three duration spellings, the load
// fallback, the intensity target) live in @/ai/planned-structure -- the
// period-review aggregation engine needs the identical reads, and a second
// copy of that production knowledge would drift silently.
