...
export function buildGenerationPrompt(
  ...
  const system = [
    ...
    `Respond with ONLY a single JSON object matching the agreed plan schema (event_type, event_date, optional narrative, and a workouts array). No prose, no code fences.`,
    `Guardrails:`,
    ...guardrails.map((g) => `- ${g}`),
  ].join("\n");
