...
// endpoint. In production the keys must be set; the apps/web boot config
// validator (apps/web/src/config.ts) warns on missing INNGEST_EVENT_KEY but
// does not block boot so deploys without the queue layer in front of an
// event dispatch still come up.
