...
import type { PeriodKind, PeriodReviewListResponse, PeriodReviewSummary } from "@da2/shared";
 ...
 *
 * Each one costs a context assembly (a handful of athlete-scoped queries), so
 * this is a real cost knob, not a display preference. Eight weeks plus six
 * months covers "the last couple of months in detail, the last half-year in
 * outline" -- past that an athlete is browsing history, which is a paging
 * problem rather than a default-payload problem.
 */
...
async function summarize(
  ...
  periodKey: string,
  narrated: Set<string>,
): Promise<PeriodReviewSummary | null> {
  try {
    ...
    return {
      ...
      hasNarration: narrated.has(`${kind}:${periodKey}`),
    };
  } catch (err) {
    // One period failing must not take the whole list down.
    console.warn("[reviews.list] period summary failed", {
      athlete_id: athleteId,
      kind,
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const summaries = await Promise.all(
    wanted.map((p) => summarize(db, user.id, timezone, p.kind, p.key, narrated)),
  );

  // Newest first across both cadences. `enumerateRecentPeriods` already
  // returns each cadence newest-first; sorting on the resolved start date
  // interleaves them correctly rather than showing all weeks then all months.
  const periods = summaries
    .filter((s): s is PeriodReviewSummary => s !== null)
    .sort(
      (a, b) =>
