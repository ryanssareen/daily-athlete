...
- `apps/web/src/db/__tests__/plans.test.ts` (new, DB-backed per existing `db/__tests__` convention)

**Approach:** Export `listPlans(admin, athleteId)`, `getPlan(admin, athleteId, planId)`, `archivePlan(admin, athleteId, planId)`, `softDeletePlan(admin, athleteId, planId)`. All four take the admin Supabase client plus the resolved athlete id (never trust a client-supplied id for the `athleteId` filter). `listPlans`/`getPlan` filter `deleted_at IS NULL`; `archivePlan` only transitions `status` when current status is `'active'` or already `'archived'` (idempotent) and current `deleted_at IS NULL` — never flips `archived` back to `active`. `softDeletePlan` sets `deleted_at` only if currently null; if already set, treats as a no-op success. Both return a discriminated result (`{ ok: true, plan }` | `{ ok: false, reason: "not_found" }`) so routes can map to HTTP status without re-deriving the logic.

**Technical design:**
```
archivePlan(admin, athleteId, planId):
  row = SELECT * FROM plans WHERE id = planId AND athlete_id = athleteId AND deleted_at IS NULL
  if !row: return { ok: false, reason: "not_found" }
  if row.status == "archived": return { ok: true, plan: row }   // idempotent no-op
  UPDATE plans SET status = 'archived', archived_at = now() WHERE id = planId
  return { ok: true, plan: <updated row> }
```
(Directional — exact query builder calls follow the Supabase client patterns already used in `apps/web/app/api/weekly-review/route.ts`.)
...
**Verification:** All scenarios above pass against local Supabase Postgres; `plans_one_active_per_athlete` partial index is never violated by any test case.
...
**Test scenarios:**
- Authenticated athlete with 3 plans (2 active-history, 1 archived) gets all 3, newest first.
...
**Patterns to follow:** `apps/web/app/api/coach/links/[id]/archive/route.ts` — closest existing analog for an archive-action PATCH route (auth → ownership-scoped lookup → update → response).
...
**Approach:** `apps/web/app/(athlete)/plans/page.tsx` is a Server Component that does the initial fetch (mirrors `apps/web/app/(athlete)/plan/page.tsx`'s existing shape) and renders `PlanHistoryList`, a client component showing each plan's status, event type/date, and created date, each linking to `/plans/[id]`. The detail page fetches one plan and renders `PlanActions` — archive/delete buttons using the two-step inline confirm pattern from `apps/web/src/components/coach-disconnect.tsx` (neutral button → confirming state with warning copy + `useTransition` for pending state), not a modal. Copy warns explicitly when the plan being archived/deleted is the athlete's current active plan ("This is your current plan — archiving it will leave you without an active plan until you generate a new one."). Both actions call the U4/U5 routes via plain `fetch` (no React Query/SWR anywhere in this repo — stay consistent) and route back to `/plans` on success.
...
- No new migration is required — `status`, `archived_at`, and `deleted_at` all already exist on `plans` per `supabase/migrations/0007_plans_and_planned_workouts.sql`.
