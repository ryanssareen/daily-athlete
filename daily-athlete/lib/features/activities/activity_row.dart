...
class ActivityRow extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ListTile(
      ...
      trailing: Column(
        ...
        children: [
          if (_keyMetric(workout, distanceUnit).isNotEmpty)
            Text(
              _keyMetric(workout, distanceUnit),
  ...
  static String _keyMetric(CompletedWorkoutRow w, String distanceUnit) {
    if (w.distanceM != null && w.distanceM! > 0) {
      return formatDistanceM(w.distanceM!, distanceUnit);
    }
    if (w.durationS != null) {
...
/// Run/bike/swim with distance → "X.X km" (or "X.X mi" per [distanceUnit]).
/// Strength/other (or no distance) → formatted duration.
/// Exported for unit tests.
String keyMetricFor(CompletedWorkoutRow w, {String distanceUnit = 'km'}) {
  if (w.distanceM != null && w.distanceM! > 0) {
    return formatDistanceM(w.distanceM!, distanceUnit);
  }
  if (w.durationS != null) {
